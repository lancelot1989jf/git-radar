...
请读取仓库中的 `data/latest_candidates.json`，生成一份中文 Markdown 报告，并写入调用方指定的输出文件。

报告目标：
帮助我从 GitHub 上近期最火、涨星最快、最值得关注的金融/量化/自动化交易项目中获取产品、技术架构、AI Agent、交易系统、数据工程和风控方面的灵感。

重要限制：
- 只基于 `data/latest_candidates.json` 和仓库内已有文件分析。
...
请严格输出 Markdown，不要输出解释性前言，不要包含 Codex 自己的运行日志。Markdown 表格不要过宽，必要时压缩字段。

报告日期请优先从 `current_snapshot` 文件名提取，例如 `2026-05-04.json` 对应 `2026-05-04`；如果该字段缺失，再从 `generated_at` 提取日期。不要保留 `{{今日日期}}` 这样的模板变量。
